#!/usr/bin/env python3
"""
dnn_lowmass.py  —  DNN classifier for YH→bbγγ low-mass resonance search
=========================================================================
...
DNN_PARAMS = dict(
    hidden_layers       = [128, 64, 32],  # keep 3 hidden layers
...
OPTIMIZATION_SETTINGS = dict(
    plot_correlation_matrix   = False,  # expensive for large samples
    plot_feature_importance   = False,  # very expensive (N_features full re-inferences)
    dataloader_num_workers    = max(1, min(8, (os.cpu_count() or 2) // 2)),
...
def main():
    ...
    batch_size = (
        TRAINING_PARAMS["batch_size_gpu"]
        if device.type == "cuda"
        else TRAINING_PARAMS["batch_size_cpu"]
    )
    num_workers = OPTIMIZATION_SETTINGS["dataloader_num_workers"]
    use_persistent_workers = (
        OPTIMIZATION_SETTINGS["dataloader_persistent_workers"] and num_workers > 0
    )
    ...
    for feat_name, feat_list in FEATURE_SETS.items():
        ...
        OUTPUT_DIR = (
            ...
            f"hhbbgg_AwkwardAnalyzer/ML_Application/low_mass/dnn_output/optimized/3layers_{SIG_LABEL}/"
        )
