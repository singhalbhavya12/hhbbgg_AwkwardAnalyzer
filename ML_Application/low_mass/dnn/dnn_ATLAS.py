#!/usr/bin/env python3
"""
dnn_lowmass.py  —  DNN classifier for HH→bbγγ low-mass resonance search
=========================================================================

Trains a PyTorch DNN to separate  NMSSM  X → YH → bbγγ  signal
from non-resonant backgrounds (γγ+jets, γ+jets).

How to run (on lxplus, with LCG_106):
    source /cvmfs/sft.cern.ch/lcg/views/LCG_106/x86_64-el9-gcc13-opt/setup.sh
    python3 dnn_lowmass.py
...
# --- Signal mass points -------------------------------------------------------
# Extend/trim this map to control MX,MY grid globally.
SIGNAL_POINTS_BY_X = {
...
# --- DNN hyper-parameters (2-feature ATLAS-like training) -------------------
DNN_PARAMS = dict(
    hidden_layers       = [32, 16],  # two hidden layers for 2 inputs
    dropout             = 0.2,        # moderate dropout (not too aggressive for small net)
    batch_norm          = False,      # see explanation below
    activation          = "ReLU",
)
#
# WHY batch_norm=False for this 2-feature model:
# ────────────────────────────────────────────────────────────────────────────
# 1. REDUNDANCY WITH PRESCALING
#    Data is already normalized via StandardScaler before training.
#    Adding BatchNorm on top is redundant and can interfere with pre-scaling.
#
# 2. LIMITED STATISTICAL INFORMATION
#    With only 2 input features, there's little statistical diversity to normalize.
#    BatchNorm works best on high-dimensional inputs where it stabilizes training.
#    With 2 features, normalization can actually remove important scale information.
#
# 3. EXCESSIVE PARAMETERS FOR MODEL SIZE
#    Each BatchNorm layer adds learnable scale/bias per feature. With limited
#    capacity (2→32→16→1), these extra parameters increase overfitting risk.
#
# 4. SHALLOW NETWORK
#    The 32→16→1 architecture is already quite compact. Internal covariate shift
#    (which BatchNorm addresses) is minimal in shallow, small networks.
#
# 5. TRAINING STABILITY
#    For small models with few features, BatchNorm can introduce noise or
#    instability rather than help, especially with unreliable batch statistics.
#
# Result: Simpler, more stable training with better generalization.
...
# We keep this in sync with bdt_lowmass.py so both models see the same events.
...
# ============================================================================
#  DATA  LOADING  (identical to BDT script)
# ============================================================================
...
def main():
    ...
    # present as columns in the parquet files.
    ...
    for feat_name, feat_list in FEATURE_SETS.items():
        ...
        OUTPUT_DIR = (
            ...
            f"hhbbgg_AwkwardAnalyzer/ML_Application/low_mass/dnn_output/{SIG_LABEL}/"
        )
