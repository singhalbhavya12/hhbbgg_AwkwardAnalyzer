...
def process_parquet_file(inputfile, out_files):
    ...
    for batch in parquet_file.iter_batches(batch_size=10000, columns=required_columns):
        ...
        cms_events = ak.zip(
            {
                ...
                "n_jets": tree_["n_jets"],
                 #Dijet variables
                'Res_dijet_mass': tree_['Res_dijet_mass'],
                'Res_dijet_mass_DNNreg': tree_['Res_dijet_mass_DNNreg'],
                'Res_DNNpair_dijet_mass': tree_['Res_DNNpair_dijet_mass'],
                'Res_DNNpair_dijet_mass_DNNreg': tree_['Res_DNNpair_dijet_mass_DNNreg'],
                'nonRes_dijet_mass': tree_['nonRes_dijet_mass'],
                'nonRes_dijet_mass_DNNreg': tree_['nonRes_dijet_mass_DNNreg'],
                'nonResReg_dijet_mass': tree_['nonResReg_dijet_mass'],
                'nonResReg_dijet_mass_DNNreg': tree_['nonResReg_dijet_mass_DNNreg'],
                'nonResReg_DNNpair_dijet_mass': tree_['nonResReg_DNNpair_dijet_mass'],
                'nonResReg_DNNpair_dijet_mass_DNNreg': tree_['nonResReg_DNNpair_dijet_mass_DNNreg'],
                'VBF_dijet_mass': tree_['VBF_dijet_mass']
            },
            depth_limit=1,
        )
        ...
        from regions import (
            get_mask_preselection, get_mask_selection, get_mask_bbgg,
        ...
        cms_events["bbgg"]             = get_mask_bbgg(cms_events)
        ...
        keys_to_copy = [
            ...
            # region flags
            "preselection","selection", "bbgg", "srbbgg","srbbggMET","crbbantigg","crantibbgg","crantibbantigg","sideband",
            ...
            "signal","isdata",
            #Dijet variables
            "Res_dijet_mass", "Res_dijet_mass_DNNreg", 'Res_DNNpair_dijet_mass', 'Res_DNNpair_dijet_mass_DNNreg', 'nonRes_dijet_mass', 'nonRes_dijet_mass_DNNreg', 'nonResReg_dijet_mass',
            'nonResReg_dijet_mass_DNNreg', 'nonResReg_DNNpair_dijet_mass', 'nonResReg_DNNpair_dijet_mass_DNNreg',
            'VBF_dijet_mass'
        ]
        ...
        for r in ["preselection","selection", "bbgg","srbbgg","srbbggMET","crbbantigg","crantibbgg","crantibbantigg","sideband","idmva_sideband","idmva_presel"]:
...
if "NMSSM" in inputfiles_:
    NMSSM_base_name=os.path.basename(os.path.dirname(inputfiles_))
    hist_file_path = os.path.join(output_dir, f"{NMSSM_base_name}-histograms.root")
    tree_file_path = os.path.join(output_dir, f"{NMSSM_base_name}-trees.root")
else:
    base_name = os.path.basename(inputfiles_).replace(".parquet", "").replace(".root", "")
    hist_file_path = os.path.join(output_dir, f"{base_name}-histograms.root")
    tree_file_path = os.path.join(output_dir, f"{base_name}-trees.root")
